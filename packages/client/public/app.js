...
const formatBytes = (bytes, decimals = 2) => {
  ...
  const formattedMessage = parseFloat((bytes / Math.pow(k, i)).toFixed(dm)) + " " + sizes[i];
...
const onload = () => {
  ...
  const ioClient = io.connect(BACKEND_API_URL, { withCredentials: false });
  ioClient.on("connect", (msg) => {
    console.log('[*] Connected!', ioClient.id);
    const targetUrl = BACKEND_API_URL + `?socketId=${ioClient.id}`;
